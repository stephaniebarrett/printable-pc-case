    ...
    difference()
    {
        ...
        for (i = [0:2:6])
        {
            for (j = [-tabX*i*1.5, tabX*i*1.5])
            {
                for (k = [-tabY/3,tabY/3])
                {   
                    translate([x+j,y+k,RACK_FLOOR_THICKNESS]) hole_through(name="M3",h=M3x10HeadHeight, $fn=32);
                    translate([x+j,y+k,M3x10NutHeight]) nutcatch_parallel("M3", clh=0.1);
                }
            }
        }
    }
...
module draw_rack_right_wall(rackUnits, depth)
{
    ...
    difference()
    {
        ...
        translate([rack_outer_dims[0]-mountingRailThickness(MOUNTING_SCREW_TYPE)/2, RACK_WALL_THICKNESS+mountingRailThickness(MOUNTING_SCREW_TYPE)/2, rack_inner_dims[2]]) union()
        {
            ...
            {
                translate([0,0,RACK_FLOOR_THICKNESS+EPSILON]) rotate([0,0,90]) hole_through(name="M3", l=RACK_FLOOR_THICKNESS, cld=0.1, $fn=32);
                rotate([0,0,180]) nutcatch_sidecut("M3");
            }
        ...
        for (z = [RACK_FLOOR_THICKNESS*3, rack_outer_dims[2]-RACK_FLOOR_THICKNESS*3])
        {
            ...
            {
                translate([0,0,-mountingRailThickness(MOUNTING_SCREW_TYPE)*2+M3x10NutHeight]) nutcatch_parallel("M3", clh=0.1);
                hole_through(name="M3", l=mountingRailThickness(MOUNTING_SCREW_TYPE)*2, cld=0.1, h=M3x10HeadHeight, hcld=0.4, $fn=32);
            }
        }
...
module draw_rack_rear(rackUnits, depth)
{
    ...
    difference()
    {
        ...
        for (x = [rack_outer_dims[0]/3-mountingRailThickness(MOUNTING_SCREW_TYPE)+0.5, rack_outer_dims[0]-rack_outer_dims[0]/3-mountingRailThickness(MOUNTING_SCREW_TYPE)+0.5])
        {
            ...
            {
                translate([mountSize[0]*2-M3x10NutHeight-0.1,0,0]) rotate([0,-90,0]) nutcatch_parallel("M3", clh=0.1);
                rotate([0,-90,0]) hole_through(name="M3", l=mountingRailThickness(MOUNTING_SCREW_TYPE)*2, cld=0.1, h=M3x10HeadHeight, hcld=0.4, $fn=32);
            }
        }
        // roof mounting holes
        ...
        {
            translate([0,0,EPSILON]) rotate([0,0,90]) hole_through(name="M3", l=RACK_FLOOR_THICKNESS, cld=0.1, $fn=32);
            translate([0,0,-RACK_WALL_THICKNESS]) rotate([0,0,-90]) nutcatch_sidecut("M3");
        }
