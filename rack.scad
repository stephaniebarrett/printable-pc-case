...
module draw_rack_rear(rackUnits, depth)
{
    ...
    difference()
    {
        ...
        for (i = [-1:1])
        {
            translate([x+(2*i)+((fanSize+20)*i), y, z]) draw_fan_through_hole(fanSize,fanSize,FAN_DEPTH);
            translate([x+(2*i)+((fanSize+20)*i), y, z]) draw_fan_mounting_holes(fanSize,FAN_DEPTH);
        }
    }
}
